package set_default_branch_test
...
func setupAction(t *testing.T, transport *httpmock.MockTransport) action.Action {
	...
	a := set_default_branch.NewAction()
...
func newInvokeConfig(projectID, branchID string) tfsdk.Config {
	s := getActionSchema()
...
func TestSetDefaultBranchAction_Invoke(t *testing.T) {
	transport := httpmock.NewMockTransport()

	transport.RegisterResponder(http.MethodPost,
		"https://neon.example.com/api/v2/projects/proj-001/branches/br-001/set_as_default",
	...
	a.Invoke(context.Background(), action.InvokeRequest{
		Config: newInvokeConfig("proj-001", "br-001"),
	}, resp)
...
func TestSetDefaultBranchAction_APIError(t *testing.T) {
	transport := httpmock.NewMockTransport()
