...
func setupEphemeral(t *testing.T, transport *httpmock.MockTransport) ephemeral.EphemeralResource {
	...
	e := neon_auth_oauth_provider.NewEphemeralResource()
...
func newOpenConfig(projectID, branchID, id string) tfsdk.Config {
...
func TestNeonAuthOauthProviderEphemeral_Open(t *testing.T) {
	...
	e.Open(context.Background(), ephemeral.OpenRequest{
		Config: newOpenConfig("test-project-id", "br-test-001", "google"),
	}, resp)
...
func TestNeonAuthOauthProviderEphemeral_NotFound(t *testing.T) {
	...
	transport.RegisterResponder(http.MethodGet,
		"https://neon.example.com/api/v2/projects/test-project-id/branches/br-test-001/auth/oauth_providers",
		testutil.JSONResponder(200, `{"providers": []}`),
	)
...
func TestNeonAuthOauthProviderEphemeral_APIError(t *testing.T) {
	transport := httpmock.NewMockTransport()
