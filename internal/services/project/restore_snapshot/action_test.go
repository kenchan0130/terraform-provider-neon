...
func newInvokeConfig(projectID, snapshotID string, name, targetBranchID *string, finalizeRestore *bool) tfsdk.Config {
	s := getActionSchema()
	schemaType := s.Type().TerraformType(context.Background())

	nameValue := tftypes.NewValue(tftypes.String, nil)
	if name != nil {
		nameValue = tftypes.NewValue(tftypes.String, *name)
	}

	targetBranchIDValue := tftypes.NewValue(tftypes.String, nil)
	if targetBranchID != nil {
		targetBranchIDValue = tftypes.NewValue(tftypes.String, *targetBranchID)
	}

	finalizeRestoreValue := tftypes.NewValue(tftypes.Bool, nil)
	if finalizeRestore != nil {
		finalizeRestoreValue = tftypes.NewValue(tftypes.Bool, *finalizeRestore)
	}
	...
	return tfsdk.Config{
		...
		Raw: tftypes.NewValue(schemaType, map[string]tftypes.Value{
			...
			"name":             nameValue,
			"target_branch_id": targetBranchIDValue,
			"finalize_restore": finalizeRestoreValue,
		}),
	}
}

func TestRestoreSnapshotAction_Invoke(t *testing.T) {
	transport := httpmock.NewMockTransport()

	transport.RegisterResponder(http.MethodPost,
		"https://neon.example.com/api/v2/projects/proj-001/snapshots/snap-001/restore",
		testutil.JSONResponder(200, `{
			"branch": {
				"id":"br-001","project_id":"proj-001","name":"restored","primary":false,"default":false,
	...
	a.Invoke(context.Background(), action.InvokeRequest{
		Config: newInvokeConfig("proj-001", "snap-001", nil, nil, nil),
	}, resp)
...
func TestRestoreSnapshotAction_WithOptionalFields(t *testing.T) {
	transport := httpmock.NewMockTransport()
	...
	transport.RegisterResponder(http.MethodPost,
		...
		testutil.JSONResponder(200, `{
			"branch": {
				"id":"br-001","project_id":"proj-001","name":"my-restored-branch","primary":false,"default":false,
				"created_at":"2025-01-01T00:00:00Z","updated_at":"2025-01-01T00:00:00Z",
				"state_changed_at":"2025-01-01T00:00:00Z","current_state":"ready",
				"creation_source":"console","protected":false,
				"cpu_used_sec":0,"compute_time_seconds":0,"active_time_seconds":0,
	...
	name := "my-restored-branch"
	targetBranchID := "br-002"
	finalizeRestore := true
	resp := &action.InvokeResponse{}
	a.Invoke(context.Background(), action.InvokeRequest{
		Config: newInvokeConfig("proj-001", "snap-001", &name, &targetBranchID, &finalizeRestore),
	}, resp)
...
func TestRestoreSnapshotAction_APIError(t *testing.T) {
	transport := httpmock.NewMockTransport()
