...
func newInvokeConfig(projectID string) tfsdk.Config {
	s := getActionSchema()
...
func TestRecoverProjectAction_Invoke(t *testing.T) {
	transport := httpmock.NewMockTransport()

	transport.RegisterResponder(http.MethodPost,
		"https://neon.example.com/api/v2/projects/proj-001/recover",
		testutil.JSONResponder(200, `{
			"project": {
				"id":"proj-001","platform_id":"aws","region_id":"us-east-1","name":"my-project",
				"pg_version":16,"proxy_host":"us-east-1.aws.neon.tech",
				"branch_logical_size_limit":0,"branch_logical_size_limit_bytes":0,
				"store_passwords":true,"cpu_used_sec":0,
				"compute_time_seconds":0,"active_time_seconds":0,
	...
	a.Invoke(context.Background(), action.InvokeRequest{
		Config: newInvokeConfig("proj-001"),
	}, resp)
...
func TestRecoverProjectAction_APIError(t *testing.T) {
	transport := httpmock.NewMockTransport()
