...
- **Never echo server-owned Optional+Computed values back in Update** (config-gating, see #61): with `UseStateForUnknown`, values the server populated (never configured) sit in the plan and a naive "serialize the whole plan" Update sends them back — the API may reject them (e.g. `settings.maintenance_window` → 400 on Free plans). Rule: **config decides whether a field is included; the plan supplies the value.** Gate per leaf (a configured parent object does not mean its unconfigured leaves get sent). Null config = absent; UNKNOWN config = configured (take the value from the apply-time-resolved plan) — only null proves absence. Never attach a nested object with zero included leaves (`"allowed_ips": {}` can mean "allow all IPs"). Reference implementation: `internal/services/project/project/resource.go` (`buildProjectSettingsRequest` and friends, `(value, included bool)` returns).
...
- **Never put Required children inside an Optional+Computed nested attribute** (see #61): when the server populates the object but config doesn't set it, Terraform core builds the proposed new state taking Required children from config (null), which never matches the `UseStateForUnknown`-restored prior state — every plan is then flagged as changed and `UnknownOnResourceChange` attributes (`updated_at`) produce a **perpetual no-op diff** that survives apply forever. Children must be Optional+Computed with their own `UseStateForUnknown`, using `AlsoRequires` validators for all-or-nothing configuration (see `projectMaintenanceWindowSchema`). Note `AlsoRequires` skips null values, so an empty object (`maintenance_window = {}`) passes validation — the request builder must gate each child on config (config-gating handles this).
